...
with open("../data/raw_data/coassembly/all.coassembly_proteins_1st_ClusterDB_repseq.fasta", "r") as fasta_file:
...
# Save to TSV
print('success, storing at ../out/aaseqs/orf_sequences_09_K00362.tsv')
df = pd.DataFrame(orf_sequence_pairs)
df.to_csv('../out/aaseqs/orf_sequences_K00362_90.tsv', sep='\t', index=False)

missing_ids = cluster_IDs - found_ids
print(f"{len(missing_ids)} IDs in cluster list not found in FASTA:")
