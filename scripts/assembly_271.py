...
def main():
    ...
    cluster_IDs = pd.read_csv('../out/cluster_ids_271.tsv', sep='\t', header=None)
    ...
    def find_cluster(orf):
        prefix = orf.split('.')[0]
        df = pd.read_csv('../out/new_cluster_map.tsv', sep ='\t', header=None)
    ...
    print(data)
    np.savetxt(f"../out/T0data_271_nar.tsv", data, delimiter = '\t', fmt = '%0.6f')
    ...
    for soil in soils: 
        ...
        print(data)
        np.savetxt(f"../out/{soil}data_271_nar.tsv", data, delimiter = '\t', fmt = '%0.6f')
