'''
Assembly using the original annotation data, at 0.9 similiarity from MMSeq
Input: a KO number (eg usage: python assembly09.py --ko 'K00370')
...
def main(): 
    ...
    def find_cluster_T0(orf):
        prefix = orf.split('.')[0]
        FPATH = f"../data/subset_{args.ko}/T0.coassembly_annotations_{args.ko}.tsv"
    ...
    def find_cluster(orf):
        prefix = orf.split('.')[0]
        FPATH = f"../data/subset_{args.ko}/{soil}.coassembly_annotations_{args.ko}.tsv"
        df = pd.read_csv(FPATH, sep='\t', header=None)
        cluster = df[df[0] == orf][2].tolist()
        return cluster[0]
    ...
    print(data)
    np.savetxt(f"../out/T0data_09_{args.ko}.tsv", data, delimiter = '\t', fmt = '%0.6f')
    ...
    for soil in soils: 
        ...
        print(data)
        np.savetxt(f"../out/{soil}data_09_{args.ko}.tsv", data, delimiter = '\t', fmt = '%0.6f')
